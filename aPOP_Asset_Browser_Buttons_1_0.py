bl_info = {
    "name": "Asset Browser Display Type Buttons",
    "author": "aPOP",
    "version": (1, 0, 0),
    "blender": (4, 4, 0),
...
class FILEBROWSER_OT_display_thumbnails(Operator):
    ...
    def execute(self, context):
        print("Thumbnail operator executed!")
...
def draw_display_buttons(self, context):
    ...
    if context.space_data and context.space_data.type == 'FILE_BROWSER':
        ...
        # Debug print to check if thumbnail operator exists
        print(f"Current display: {current_display}")
        print(f"Thumbnail operator registered: {'file.display_thumbnails' in dir(bpy.ops.file)}")
